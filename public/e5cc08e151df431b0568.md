...
tags:
  ...
  - csrf-token
private: false
updated_at: '2024-12-11T17:32:44+09:00'
...
---
こんにちは、hidepon4649です。
ポートフォリオ作成中にSpring Security6 + React + CSRFでハマったのでメモします。
...
# デバッグ対応
対応内容
- 1 .ログイン処理のリクエストを投げる前に、csrfトークンを取得
- 2 .以降、POSTリクエストする際に必ず実施する事が２点
  - 2-1 . ``` withCredentials:true``` を設定
  - 2-2 . 最初に取得したcsrfトークンをヘッダーに設定
- 以下がデバッグ対応後の実装です
```　LoginPage.tsx
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const responseCsrf = await axios.post(
        ...
        {},
        {
          // POSTリクエストする際に withCredentials:true を設定
